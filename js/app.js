...
import { ping, getConfig, setConfig, getFoodChart, logMeal, searchFood, addFood, getDraftState, setDraftState, snapshotDraftState, restoreDraftState, isDriveBackendActive, setDriveBackendActive } from './backend-router.js';
...
import { startOAuth, handleOAuthCallback, isConnected as isDriveConnected, disconnect as disconnectDrive } from './auth.js';
import {
...
// ─── GOOGLE DRIVE OAUTH (testing — not yet used for real data) ────────────────

async function handleDriveOAuthRedirect() {
  const params = new URLSearchParams(window.location.search);
  const code = params.get('code');
  const oauthState = params.get('state');
  console.log('[oauth] redirect check — code present:', !!code, 'state present:', !!oauthState);
  if (!code || !oauthState) return;

  console.log('[oauth] found code+state, exchanging for token…');
  try {
...
function setupDriveConnectionUI() {
  const statusEl = document.getElementById('gdrive-status');
  const connectBtn = document.getElementById('connect-drive-btn');
  const disconnectBtn = document.getElementById('disconnect-drive-btn');
  const backendToggle = document.getElementById('use-drive-backend-toggle');
  ...
  connectBtn.addEventListener('click', async () => {
    try {
      ...
      showToast('Could not start Google sign-in: ' + err.message, 'error');
  ...
  if (backendToggle) {
    backendToggle.checked = isDriveBackendActive();
    backendToggle.addEventListener('change', () => {
      setDriveBackendActive(backendToggle.checked);
      showToast(
        backendToggle.checked ? 'Switching to Drive backend…' : 'Switching to Apps Script backend…',
        'info'
      );
...
async function init() {
  ...
  await handleDriveOAuthRedirect();
  setupDriveConnectionUI();
...
function renderSettingsSection() {
  ...
  const colorSelect = document.getElementById('color-theme-select'); if (colorSelect) colorSelect.value = storage.get('color_theme','custom');
  ...
  if (nightlyTimeInput) nightlyTimeInput.value = state.config?.nightly_export_time || '23:50';
}
...
function setupNavigation() {
  ...
  document.getElementById('export-log-btn')?.addEventListener('click', async e => {
    showSpinner(e.target);
    try { await exportTodayLog(); } finally { hideSpinner(e.target); }
  });
  ...
  if (mobileToolsBtn && mobileToolsDropdown) {
    ...
    document.getElementById('mobile-tools-export-log')?.addEventListener('click',   () => { mobileToolsDropdown.hidden = true; document.getElementById('tools-export-log')?.click(); });
    ...
    document.getElementById('mobile-tools-export-clear')?.addEventListener('click', () => { mobileToolsDropdown.hidden = true; document.getElementById('tools-export-clear')?.click(); });
...
function setupToolsMenu() {
  ...
  document.getElementById('tools-export-log')?.addEventListener('click',   () => { document.getElementById('tools-dropdown').hidden = true; exportTodayLog(); });
  document.getElementById('tools-export-clear')?.addEventListener('click', () => { document.getElementById('tools-dropdown').hidden = true; exportTodayAndClear(); });
...
function buildMealExportPayload(slug) {
  ...
  // A meal is "eligible" if it has foods or notes — currentBG/target/ICR/
  // ISF/IOB/COB are always populated live regardless of user action, so
  // they must never make an empty meal look eligible.
  const hasData = meal.foods.length > 0 || !!(meal.notes && meal.notes.trim());
  const bolusLocked = !!bolusTimeAt;
  ...
  // Calculation fields only get included once a bolus has actually been
  // given — otherwise they're omitted even though foods/notes are present.
  if (hasData && bolusLocked) {
...
// Always builds all 6 meal slots — the per-day export Doc shows every slot
// regardless of eligibility, so callers should pass MEAL_SLUGS (a subset
// would just be missing sections in an otherwise full-day Doc).
function buildDayExportPayload(slugs = MEAL_SLUGS) {
  return slugs.map(buildMealExportPayload);
}
...
function downloadCSV(rows) {
  ...
  const filename = `Bolus Calculator - ${monthName} ${day} ${year}.csv`;
...
// Every meal slot lives in one Doc per day now, so there's a single actual
// export operation — (re)write today's Doc with whatever's currently
// eligible across all 6 slots. Returns true once the data has been saved
// somewhere (Drive, or the local CSV fallback while offline).
async function exportDayCore(meals) {
  try {
    if (state.connected) {
      showToast('Exporting…', 'info');
      const result = await logMeal({ meals, units: state.units, date: todayStr() });
...
// "Export Today's Log" — exports, leaves all 6 meal slots untouched.
async function exportTodayLog() {
  const meals = buildDayExportPayload(MEAL_SLUGS);
  if (!meals.some(m => m.hasData)) { showToast('Nothing to export today', 'error'); return; }
  await exportDayCore(meals);
}

// "Export & Clear Today" — same export, then clears just the 6 meal slots
// (not the full resetAllTransientDayState() the nightly auto-export does —
// wiping the in-progress recipe builder or food-search panel mid-day would
// be surprising for a manual action) once the export has actually landed.
async function exportTodayAndClear() {
  const meals = buildDayExportPayload(MEAL_SLUGS);
  if (!meals.some(m => m.hasData)) { showToast('Nothing to export today', 'error'); return; }
  const ok = await exportDayCore(meals);
  if (ok) MEAL_SLUGS.forEach(clearMeal);
}

// Wipes every per-day/transient/draft trace of the day that was just
// exported — the 6 meal slots, the in-progress recipe builder (including its
// Drive draft), and whatever's left sitting in the food-search/add-food
// panels — without touching persistent settings (ICR/ISF, theme,
// connections, etc.).
async function resetAllTransientDayState() {
  ...
  state.activeRecipeIndex = 0;
  if (state.connected) {
    try { await setDraftState('recipes', []); } catch (err) { console.error('Failed to clear recipe draft', err); }
  }
  renderRecipeTabs();
  renderRecipePanel();
  closeRecipePanel();
  ...
  if (standardMode) standardMode.hidden = false;
  if (customMode)   customMode.hidden   = true;
  ['custom-name', 'custom-cf', 'custom-weight', 'custom-carbs'].forEach(id => setVal(id, ''));
...
function setupExportTimer() {
  setInterval(async () => {
    if (state.config?.nightly_export_enabled === false) return;

    const targetTime = state.config?.nightly_export_time || '23:50';
    const [targetH, targetM] = targetTime.split(':').map(Number);
    const now = new Date();
    if (now.getHours() < targetH || (now.getHours() === targetH && now.getMinutes() < targetM)) return;

    // The configured time has passed — fire once per day, using TODAY's
    // date (this now runs before midnight on the day it's labeling, not
    // after — so there's no "yesterday" to compute).
    const today = todayStr();
    if (storage.get('nightly_export_ran_date') === today) return;
    storage.set('nightly_export_ran_date', today);

    const meals = buildDayExportPayload(MEAL_SLUGS);
    if (!meals.some(m => m.hasData)) return;

    try {
      if (state.connected) {
        const result = await logMeal({ meals, units: state.units, date: today });
        if (result?.success) {
          storage.set('last_export_date', today);
          await resetAllTransientDayState();
          showToast("Today's log exported automatically", 'success');
          return;
        }
      }
      downloadCSV(flattenToCSVRows(meals));
      await resetAllTransientDayState();
      showToast("Today's log saved locally (offline)", 'info');
    } catch (err) {
