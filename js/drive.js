...
const DEFAULT_CONFIG = {
  ...
  dexcom_pass: '',
  dexcom_region: 'us',
  nightly_export_enabled: true,
  nightly_export_time: '23:50',
  meals: {
...
export async function setupDriveFolders(folderName = 'Bolus Calculator') {
  let rootId = storage.get('drive_folder_id');
  if (!rootId) {
    rootId = await findOrCreateFolder(folderName);
    storage.set('drive_folder_id', rootId);
  }

  let configFolderId = storage.get('drive_config_folder_id');
  if (!configFolderId) {
    configFolderId = await findOrCreateFolder('Config', rootId);
    storage.set('drive_config_folder_id', configFolderId);
  }

  let exportsFolderId = storage.get('drive_exports_folder_id');
  if (!exportsFolderId) {
    exportsFolderId = await findOrCreateFolder('Exports', rootId);
    storage.set('drive_exports_folder_id', exportsFolderId);
  }

  let configWasNew = false;
  let configFileId = storage.get('drive_config_file_id');
  if (!configFileId) {
    const existing = await findFile('config.json', configFolderId);
    configWasNew = !existing;
    if (existing) {
      configFileId = existing.id;
    } else {
      configFileId = await writeJsonFile('config.json', DEFAULT_CONFIG, configFolderId);
    }
    storage.set('drive_config_file_id', configFileId);
  }

  let sheetId = storage.get('food_sheet_id');
  if (!sheetId) {
    const existingSheet = await findFile('Food Chart', configFolderId);
    if (existingSheet) {
      sheetId = existingSheet.id;
    } else {
      sheetId = await createSheet('Food Chart', configFolderId);
      await initFoodChartHeaders(sheetId);
    }
    storage.set('food_sheet_id', sheetId);
  }

  return { rootId, configFolderId, exportsFolderId, sheetId, configWasNew };
}
